package com.github.airext.bridge.test;

import com.adobe.fre.FREContext;
import com.adobe.fre.FREFunction;
import com.github.airext.bridge.Bridge;
import com.github.airext.bridge.exceptions.BridgeInstantiationException;
import com.github.airext.bridge.exceptions.BridgeNotFoundException;
import com.github.airext.bridge.test.functions.IsSupportedFunction;
import com.github.airext.bridge.test.functions.TestErrorFunction;
import com.github.airext.bridge.test.functions.TestResultFunction;

import java.util.HashMap;
import java.util.Map;

/**
 * Created by mobitile on 12/5/15.
 */
public class ExtensionContext extends FREContext
{
    @Override
    public Map<String, FREFunction> getFunctions()
    {
        HashMap<String, FREFunction> functions = new HashMap<String, FREFunction>();

        functions.put("isSupported", new IsSupportedFunction());
        functions.put("testResult", new TestResultFunction());
        functions.put("testError", new TestErrorFunction());

        try
        {
            Bridge.setup(functions);
        }
        catch (BridgeNotFoundException e)
        {
            e.printStackTrace();
        }
        catch (BridgeInstantiationException e)
        {
            e.printStackTrace();
        }

        return functions;
    }

    @Override
    public void dispose()
    {

    }
}
